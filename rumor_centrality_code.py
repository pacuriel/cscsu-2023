...
def rumor_centrality_root(I, v, return_all_values=True):
    ...
    for u in nodes_by_depth[::-1]:
        ...
        if u == v:
            # u is root
            r[u] = int(math.factorial(N) // list_product([p[u][j] for j in children_u])) #converting to integer and using integer division
        else:
            parent_u = list(G.in_edges(u))[0][0]
            r[u] = int(r[parent_u] * t[parent_u][u] // (N - t[parent_u][u])) #converting to integer and using integer division
    for u in nodes_by_depth:
        r[u] = r[u] // len(G) #converting to integer and using integer division
...
def rumor_centrality(I, G=None, v=None, normalize=True, only_roots=False):
    ...
    for node in rumor_centrality_dict:
        rumor_centrality_dict[node] = int(rumor_centrality_dict[node] // len(I)) #converting to integer and using integer division
    if normalize:
